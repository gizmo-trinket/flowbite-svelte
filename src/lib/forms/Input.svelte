...
<script lang="ts">
  ...
  $: {
    ...
    inputClass = twMerge([defaultClass, inputPadding[_size], ($$slots.left && leftPadding[_size]) || ((clearable || $$slots.right) && rightPadding[_size]), ringClasses[color], colorClasses[_color], borderClasses[_color], textSizes[_size], group || 'rounded-none', group && 'first:rounded-s-none last:rounded-e-none', group && '[&:not(:first-child)]:-ms-px', $$props.class]);
  }
